    ...
    for (крок, затримка) in [150u64, 400, 250, 600].iter().enumerate() {
        ...
        match downloader_core::state::load(&tmp.0) {
            ...
            Err(downloader_core::state::LoadError::Absent) => {
                // ⚠️ Відсутній стан означає **дві різні речі**, і плутати їх не
                // можна: або качання завершилось і прибрало за собою, або
                // перший чекпоінт просто ще не встиг лягти на диск.
                //
                // Розрізняє їх те, чи бачили ми стан раніше: файл стану
                // прибирається лише після успіху, тож «був і зник» — це
                // завершення, а «не було жодного разу» — це ми вбили процес
                // швидше за перший запис.
                if виміри.is_empty() {
                    continue;
                }
